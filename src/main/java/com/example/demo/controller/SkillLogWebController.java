...
public class SkillLogWebController {
    ...
    @PostMapping("/new")
    public String save(@ModelAttribute SkillLog skillLog, @RequestParam("categoryId") Long categoryId) {
        Category category = categoryService.findById(categoryId).get(); // DBからCategoryを取得
        skillLog.setCategory(category); // 明示的にセット
    ...
    public String update(@ModelAttribute SkillLog skillLog,
        ...
        skillLogService.save(skillLog); // ID付きでsaveすればUPDATEになる
